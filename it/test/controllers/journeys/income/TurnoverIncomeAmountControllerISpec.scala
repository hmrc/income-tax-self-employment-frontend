...
class TurnoverIncomeAmountControllerISpec extends WiremockSpec with IntegrationBaseSpec {

  val url: String       = routes.TurnoverIncomeAmountController.onPageLoad(taxYear, businessId, NormalMode).url
  val submitUrl: String = routes.TurnoverIncomeAmountController.onSubmit(taxYear, businessId, NormalMode).url

  "GET /:taxYear/:businessId/income/turnover-amount" when {
  ...
  "POST /:taxYear/:businessId/income/turnover-amount" when {
    ...
    "the user submits a valid amount" must {
      "save the answer and redirect to the next page" in {
        ...
        val result = await(buildClient(submitUrl).post(Map[String, Seq[String]]("value" -> Seq("5000"))))
