...
class NotTaxableAmountControllerISpec extends WiremockSpec with IntegrationBaseSpec {

  val url: String       = routes.NotTaxableAmountController.onPageLoad(taxYear, businessId, NormalMode).url
  val submitUrl: String = routes.NotTaxableAmountController.onSubmit(taxYear, businessId, NormalMode).url

  "GET /:taxYear/:businessId/income/turnover-exempt/amount" when {
  ...
  "POST /:taxYear/:businessId/income/turnover-exempt/amount" when {
    ...
    "the user submits a valid amount" must {
      "save the answer and redirect to the next page" in {
        ...
        val result = await(buildClient(submitUrl).post(Map[String, Seq[String]]("value" -> Seq("100"))))
