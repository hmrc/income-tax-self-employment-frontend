...
class IncomeNotCountedAsTurnoverControllerISpec extends WiremockSpec with IntegrationBaseSpec {

  val url: String       = routes.IncomeNotCountedAsTurnoverController.onPageLoad(taxYear, businessId, NormalMode).url
  val submitUrl: String = routes.IncomeNotCountedAsTurnoverController.onSubmit(taxYear, businessId, NormalMode).url

  "GET /:taxYear/:businessId/income/not-counted-turnover" when {
  ...
  "POST /:taxYear/:businessId/income/not-counted-turnover" when {
