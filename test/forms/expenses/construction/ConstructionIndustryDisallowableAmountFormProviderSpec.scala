...
class ConstructionIndustryDisallowableAmountFormProviderSpec extends CurrencyFormProviderBaseSpec("AdvertisingDisallowableAmountFormProvider") {

  private lazy val amount       = 5623.50
  private lazy val amountString = formatMoney(amount, addDecimalForWholeNumbers = false)

  override def getFormProvider(userType: UserType): Form[BigDecimal] = new ConstructionIndustryDisallowableAmountFormProvider()(userType, amount)

  override lazy val requiredError: String                     = "constructionIndustryDisallowableAmount.error.required"
