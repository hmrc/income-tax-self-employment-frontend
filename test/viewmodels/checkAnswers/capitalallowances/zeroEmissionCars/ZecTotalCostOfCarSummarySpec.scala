...
class ZecTotalCostOfCarSummarySpec extends SummaryBaseSpec("ZecTotalCostOfCarSummary") {
  ...
  override val testKey: UserType => Text = (userType: UserType) => Text(messages(s"zecTotalCostOfCar.subHeading.$userType"))

  override val testValue: HtmlContent = HtmlContent("£20.00")

  override def buildSummaryListRow(userAnswers: UserAnswers, userType: UserType): Option[SummaryListRow] =
    ZecTotalCostOfCarSummary.row(userAnswers, taxYear, businessId, userType)(messages)
