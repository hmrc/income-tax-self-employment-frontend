...
import base.SpecBase.{ToFutureOps, businessId, convertScalaFuture, emptyUserAnswers}
...
class ZegvHowMuchDoYouWantToClaimServiceSpec extends AnyWordSpecLike with ArgumentMatchersSugar with IdiomaticMockito {
  ...
  "submitAnswer" should {
    "return a UserAnswers with the updated value" in {
      mockService.persistAnswer(eqTo(businessId), *, eqTo(BigDecimal(10.0)), eqTo(ZegvClaimAmountPage))(*) returns expectedAnswers.asFuture
