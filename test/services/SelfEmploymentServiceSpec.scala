...
class SelfEmploymentServiceSpec extends SpecBase with MockitoSugar with ArgumentMatchersSugar {
  ...
  "getCompletedTradeDetails" - {
    "should return a Right(Seq(TradesJourneyStatuses)) when this is returned from the backend" in {
      when(mockConnector.getCompletedTradesWithStatuses(meq(nino.value), meq(taxYear), meq(mtditid))(any, any)) thenReturn Future(
        Right(aSequenceTadesJourneyStatusesModel))

      val result = await(service.getCompletedTradeDetails(nino, currTaxYear, Mtditid(mtditid)).value)
  ...
  "getBusinessAccountingType" - {
    "should return a BusinessID's accounting type in a Right when this is returned from the backend" in {
      when(mockConnector.getBusiness(meq(nino.value), meq(businessIdAccrual), meq(mtditid))(any, any)) thenReturn Future(Right(aBusinessData))
      when(mockConnector.getBusiness(meq(nino.value), meq(businessIdCash), meq(mtditid))(any, any)) thenReturn Future(
        Right(aBusinessDataCashAccounting))
    ...
    "should return a Left(HttpError) when" - {
      ...
      "an empty sequence is returned from the backend" in {
        when(mockConnector.getBusiness(meq(nino.value), meq(businessIdAccrual), meq(mtditid))(any, any)) thenReturn Future(Right(Seq.empty))
      ...
      "a Left(HttpError) is returned from the backend" in {
        when(mockConnector.getBusiness(meq(nino.value), meq(businessIdAccrual), meq(mtditid))(any, any)) thenReturn Future(
