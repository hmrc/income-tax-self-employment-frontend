...
class DisallowableSubcontractorCostsSpec extends AnyFreeSpec with Matchers with ScalaCheckPropertyChecks with OptionValues {
  ...
  "DisallowableSubcontractorCosts" - {
    ...
    "must deserialise valid values" in {
      ...
      forAll(gen) { disallowableSubcontractorCosts =>
        JsString(disallowableSubcontractorCosts.toString)
          .validate[DisallowableSubcontractorCosts]
          .asOpt
          .value mustEqual disallowableSubcontractorCosts
      }
    }
    ...
    "must serialise" in {
      ...
      val gen = Gen.oneOf(DisallowableSubcontractorCosts.values)
