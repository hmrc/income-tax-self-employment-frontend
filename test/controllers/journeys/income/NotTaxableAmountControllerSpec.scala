...
class NotTaxableAmountControllerSpec extends SpecBase with MockitoSugar {
  ...
  def notTaxableAmountRoute(isPost: Boolean, mode: Mode): String =
    if (isPost) NotTaxableAmountController.onSubmit(taxYear, mode).url
    else NotTaxableAmountController.onPageLoad(taxYear, mode).url
  ...
  "NotTaxableAmount Controller" - {
    ...
    "onPageLoad" - {
      ...
      userScenarios.foreach { userScenario =>
        s"when language is ${isWelshToString(userScenario.isWelsh)} and user is an ${isAgentToString(userScenario.isAgent)}" - {
          "must return OK and the correct view for a GET" in {
            ...
            running(application) {
              val request = buildRequest(GET, notTaxableAmountRoute(false, NormalMode), userScenario.isAgent)
          ...
          "must populate the view correctly on a GET when the question has previously been answered" in {
            ...
            val userAnswers = UserAnswers(userAnswersId).set(NotTaxableAmountPage, validAnswer).success.value

            val application = applicationBuilder(userAnswers = Some(userAnswers), userScenario.isAgent).build()
            implicit val messagesApi = application.injector.instanceOf[MessagesApi]

            running(application) {
              val request = buildRequest(GET, notTaxableAmountRoute(false, CheckMode), userScenario.isAgent)
      ...
      "must redirect to Journey Recovery for a GET if no existing data is found" ignore { //TODO unignore when RequireData is implemented
        ...
        running(application) {
          val request = buildRequest(GET, notTaxableAmountRoute(false, NormalMode), false)
    ...
    "onSubmit" - {
      ...
      "must redirect to the next page when valid data is submitted" in {
        ...
        running(application) {
          val request =
            buildRequest(POST, notTaxableAmountRoute(true, NormalMode), false)
      ...
      userScenarios.foreach { userScenario =>
        s"when language is ${isWelshToString(userScenario.isWelsh)} and user is an ${isAgentToString(userScenario.isAgent)}" - {
          "must return a Bad Request and errors when an empty form is submitted" in {
            ...
            running(application) {
              val request =
                buildRequest(POST, notTaxableAmountRoute(true, NormalMode), userScenario.isAgent)
