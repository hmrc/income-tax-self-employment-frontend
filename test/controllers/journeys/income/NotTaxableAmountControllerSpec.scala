  ...
  "NotTaxableAmount Controller" - {
      ...
      userScenarios.foreach { userScenario =>
        s"when language is ${getLanguage(userScenario.isWelsh)} and user is an ${authUserType(userScenario.isAgent)}" - {
          "must return OK and the correct view for a GET" in {
            ...
            running(application) {
              val request = FakeRequest(GET, NotTaxableAmountController.onPageLoad(taxYear, NormalMode).url)
          ...
          "must populate the view correctly on a GET when the question has previously been answered" in {
            ...
            running(application) {
              val request = FakeRequest(GET, NotTaxableAmountController.onPageLoad(taxYear, CheckMode).url)
    ...
    "onSubmit" - {
      ...
      "must redirect to the next page when valid data is submitted" in {
        ...
        running(application) {
          val request =
            FakeRequest(POST, NotTaxableAmountController.onSubmit(taxYear, NormalMode).url)
