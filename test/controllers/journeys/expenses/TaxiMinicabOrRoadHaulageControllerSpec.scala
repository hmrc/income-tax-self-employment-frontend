        ...
        s"when language is ${getLanguage(userScenario.isWelsh)} and user is an ${authUserType(userScenario.isAgent)}" - {
          ...
          "must populate the view correctly on a GET when the question has previously been answered" in {
            ...
            running(application) {
              ...
              val expectedResult =
                view(userScenario.form.fill(TaxiMinicabOrRoadHaulage.values.head), NormalMode, authUserType(userScenario.isAgent), taxYear, stubbedBusinessId)(
                  request,
                  messages(application, userScenario.isWelsh)).toString
        ...
        s"when language is ${getLanguage(userScenario.isWelsh)} and user is an ${authUserType(userScenario.isAgent)}" - {
          "must return a Bad Request and errors when an empty form is submitted" in {
            ...
            running(application) {
              ...
              val expectedResult =
                view(boundForm, NormalMode, authUserType(userScenario.isAgent), taxYear, stubbedBusinessId)(
                  request,
                  messages(application, userScenario.isWelsh)).toString
