...
class EntertainmentAmountControllerSpec
  ...
  lazy val onPageLoadRoute = routes.EntertainmentAmountController.onPageLoad(TaxYear(taxYear), BusinessId(stubbedBusinessId), NormalMode).url
  lazy val onSubmitRoute   = routes.EntertainmentAmountController.onSubmit(TaxYear(taxYear), BusinessId(stubbedBusinessId), NormalMode).url
