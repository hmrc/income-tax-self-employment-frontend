...
class DisallowableSubcontractorCostsControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val disallowableSubcontractorCostsRoute =
    controllers.journeys.expenses.routes.DisallowableSubcontractorCostsController.onPageLoad(NormalMode).url

  val formProvider = new DisallowableSubcontractorCostsFormProvider()

  case class UserScenario(isWelsh: Boolean, isAgent: Boolean, form: Form[DisallowableSubcontractorCosts])
      ...
      userScenarios.foreach { userScenario =>
        s"when language is ${getLanguage(userScenario.isWelsh)} and user is an ${userType(userScenario.isAgent)}" - {
          ...
          "must populate the view correctly on a GET when the question has previously been answered" in {
            ...
            val userAnswers = UserAnswers(userAnswersId).set(DisallowableSubcontractorCostsPage,
              DisallowableSubcontractorCosts.values.head).success.value
            ...
            running(application) {
              ...
              val expectedResult = view(userScenario.form.fill(DisallowableSubcontractorCosts.values.head),
                NormalMode,
          ...
          "must return a Bad Request and errors when invalid data is submitted" in {
            ...
            running(application) {
              ...
              contentAsString(result) mustEqual view(boundForm, NormalMode,
                userType(userScenario.isAgent))(request, messages(application)).toString
            }
          }
