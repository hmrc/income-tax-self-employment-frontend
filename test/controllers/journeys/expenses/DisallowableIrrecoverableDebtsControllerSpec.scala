...
class DisallowableIrrecoverableDebtsControllerSpec extends SpecBase with MockitoSugar {
  ...
  "DisallowableIrrecoverableDebts Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(DisallowableIrrecoverableDebtsPage, DisallowableIrrecoverableDebts.values.head).success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(DisallowableIrrecoverableDebts.values.head), NormalMode)(
          request,
