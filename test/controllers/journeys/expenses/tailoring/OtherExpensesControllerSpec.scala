...
class OtherExpensesControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val otherExpensesRoute = OtherExpensesController.onPageLoad(taxYear, businessId, NormalMode).url
        ...
        s"when language is ${getLanguage(userScenario.isWelsh)} and user is an ${userType(userScenario.isAgent)}" - {
          ...
          "must populate the view correctly on a GET when the question has previously been answered" in {
            ...
            running(application) {
              ...
              val expectedResult =
                view(userScenario.form.fill(OtherExpenses.values.head), NormalMode, userType(userScenario.isAgent), taxYear, businessId)(
                  request,
                  messages(application, userScenario.isWelsh)).toString
