...
import controllers.journeys.expenses.tailoring.routes.TaxiMinicabOrRoadHaulageController
import controllers.standard.routes.JourneyRecoveryController
import forms.expenses.tailoring.TaxiMinicabOrRoadHaulageFormProvider
...
import views.html.journeys.expenses.tailoring.TaxiMinicabOrRoadHaulageView
      ...
      userScenarios.foreach { userScenario =>
        s"when language is ${getLanguage(userScenario.isWelsh)} and user is an ${userType(userScenario.isAgent)}" - {
          ...
          "must populate the view correctly on a GET when the question has previously been answered" in {
            ...
            val userAnswers = UserAnswers(userAnswersId).set(TaxiMinicabOrRoadHaulagePage, TaxiMinicabOrRoadHaulage.values.head, Some(businessId)).success.value
