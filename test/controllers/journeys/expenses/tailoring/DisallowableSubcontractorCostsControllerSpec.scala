...
class DisallowableSubcontractorCostsControllerSpec extends SpecBase with MockitoSugar {
  ...
  "DisallowableSubcontractorCosts Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(DisallowableSubcontractorCostsPage, DisallowableSubcontractorCosts.values.head, Some(stubbedBusinessId)).success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(DisallowableSubcontractorCosts.values.head), NormalMode, taxYear, stubbedBusinessId)(
          request,
