...
class FinancialExpensesControllerSpec extends SpecBase with MockitoSugar {
  ...
  "FinancialExpenses Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(FinancialExpensesPage, FinancialExpenses.values.toSet).success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(FinancialExpenses.values.toSet), NormalMode)(request, messages(application)).toString
      }
    }
