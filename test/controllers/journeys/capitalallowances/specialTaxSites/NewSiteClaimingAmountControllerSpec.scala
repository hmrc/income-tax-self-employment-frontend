...
class NewSiteClaimingAmountControllerSpec extends BigDecimalGetAndPostQuestionBaseSpec("NewSiteClaimingAmountController", NewSiteClaimingAmountPage) {

  override def onPageLoadRoute: String = routes.NewSiteClaimingAmountController.onPageLoad(taxYear, businessId, 0, NormalMode).url

  override def onSubmitRoute: String = routes.NewSiteClaimingAmountController.onSubmit(taxYear, businessId, 0, NormalMode).url
  ...
  override def baseAnswers: UserAnswers = buildUserAnswers(
    Json.obj(
      ...
      "newSpecialTaxSites" -> List(Json.obj(
        ...
        "specialTaxSiteLocation"          -> Some(SpecialTaxSiteLocation(Some("name"), None, "AA11AA"))
      ))
    ))
  ...
  override def expectedView(form: Form[_], scenario: TestScenario)(implicit
    ...
    val view = application.injector.instanceOf[NewSiteClaimingAmountView]
