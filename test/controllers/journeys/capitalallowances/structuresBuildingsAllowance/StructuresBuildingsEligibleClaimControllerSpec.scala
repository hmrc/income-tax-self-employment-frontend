...
class StructuresBuildingsEligibleClaimControllerSpec
    extends RadioButtonGetAndPostQuestionBaseSpec("StructuresBuildingsEligibleClaimController", StructuresBuildingsEligibleClaimPage) {

  def onPageLoadCall: Call = routes.StructuresBuildingsEligibleClaimController.onPageLoad(taxYear, businessId, NormalMode)

  def onSubmitCall: Call = routes.StructuresBuildingsEligibleClaimController.onSubmit(taxYear, businessId, NormalMode)

  def onwardRoute: Call = routes.StructuresBuildingsEligibleClaimController.onPageLoad(taxYear, businessId, NormalMode)

  def validAnswer: Boolean = true

  def createForm(userType: UserType): Form[Boolean] = new StructuresBuildingsEligibleClaimFormProvider()(userType)
