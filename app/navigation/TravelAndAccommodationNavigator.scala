...
class TravelAndAccommodationNavigator @Inject() {
  ...
  def nextTravelExpensesPage(page: Page,
    ...
    mode match {
      ...
      case CheckMode =>
        checkTravelExpensesRouteMap(page)(model)(taxYear, businessId, userAnswers)
          .getOrElse(controllers.standard.routes.JourneyRecoveryController.onPageLoad())
    }
  ...
  private def normalIndexRoutes: Page => VehicleDetailsDb => (TaxYear, BusinessId, Index) => Option[Call] = {
    ...
    case TravelForWorkYourMileagePage =>
      _ => (taxYear, businessId, _) => Some(routes.YourFlatRateForVehicleExpensesController.onPageLoad(taxYear, businessId, NormalMode))

    case VehicleExpensesPage =>
      _ => (taxYear, businessId, _) => Some(routes.TravelAndAccommodationExpensesCYAController.onPageLoad(taxYear, businessId))
