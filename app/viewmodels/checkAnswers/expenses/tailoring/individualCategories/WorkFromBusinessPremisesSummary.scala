...
object WorkFromBusinessPremisesSummary {
  ...
  def row()(implicit messages: Messages, answers: UserAnswers, taxYear: TaxYear, businessId: BusinessId, userType: UserType): Option[SummaryListRow] =
    answers.get(WorkFromBusinessPremisesPage, Some(businessId)).map { answer =>
      buildRowString(
        formatAnswer(answer.toString),
        routes.WorkFromBusinessPremisesController.onPageLoad(taxYear, businessId, CheckMode),
        s"workFromBusinessPremises.title.$userType",
        "workFromBusinessPremises.change.hidden"
      )
    }
