...
import pages.expenses.workplaceRunningCosts.workingFromBusinessPremises.LivingAtBusinessPremisesOnePerson
import play.api.i18n.Messages
import uk.gov.hmrc.govukfrontend.views.viewmodels.summarylist.SummaryListRow
import viewmodels.checkAnswers.buildRowInt

object LivingAtBusinessPremisesOnePersonSummary {

  def row(userAnswers: UserAnswers, taxYear: TaxYear, businessId: BusinessId, userType: UserType)(implicit
                                                                                                  messages: Messages): Option[SummaryListRow] =
    userAnswers.get(LivingAtBusinessPremisesOnePerson, Some(businessId)).map { answer =>
      buildRowInt(
        answer,
        routes.PeopleLivingAtBusinessPremisesController.onPageLoad(taxYear, businessId, CheckMode),
        s"wfbpOnePerson.subHeading.$userType",
        "wfbpOnePerson.change.hidden"
      )
    }
