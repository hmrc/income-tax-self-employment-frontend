...
object EvcpOnlyForSelfEmploymentSummary {
  ...
  def row(answers: UserAnswers, taxYear: TaxYear, businessId: BusinessId, userType: UserType)(implicit messages: Messages): Option[SummaryListRow] =
    answers
      .get(EvcpOnlyForSelfEmploymentPage, businessId.some)
      .map { answer =>
        buildRowString(
          formatAnswer(answer.toString),
          routes.EvcpOnlyForSelfEmploymentController.onPageLoad(taxYear, businessId, CheckMode),
          messages(s"evcpOnlyForSelfEmployment.title.$userType"),
          "evcpOnlyForSelfEmployment.change.hidden",
          rightTextAlign = true
        )
      }
}
