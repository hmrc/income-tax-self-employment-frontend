...
object BalancingAllowanceAmountSummary {

  def row(answers: UserAnswers, taxYear: TaxYear, businessId: BusinessId, userType: UserType)(implicit messages: Messages): Option[SummaryListRow] =
    answers
      .get(BalancingAllowanceAmountPage, businessId.some)
      .map { answer =>
        buildRowBigDecimal(
          answer,
          routes.BalancingAllowanceAmountController.onPageLoad(taxYear, businessId, CheckMode),
          messages(s"balancingAllowanceAmount.title.$userType"),
          "balancingAllowanceAmount.change.hidden"
        )
      }
}
