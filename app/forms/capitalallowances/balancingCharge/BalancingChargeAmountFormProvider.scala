...
package forms.capitalallowances.balancingCharge
...
class BalancingChargeAmountFormProvider @Inject() extends Mappings with MoneyBounds {

  def apply(userType: UserType): Form[BigDecimal] =
    Form(
      "value" -> currency(s"balancingChargeAmount.error.required.$userType", "error.nonNumeric")
