...
class DepreciationController @Inject() (override val messagesApi: MessagesApi,
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData) { implicit request =>
    val preparedForm = request.userAnswers.getOrElse(UserAnswers(request.userId)).get(DepreciationPage, Some(businessId)) match {
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(
              request.userAnswers.getOrElse(UserAnswers(request.userId)).set(DepreciationPage, value, Some(businessId)))
            _ <- sessionRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(DepreciationPage, mode, updatedAnswers))
      )
  }
