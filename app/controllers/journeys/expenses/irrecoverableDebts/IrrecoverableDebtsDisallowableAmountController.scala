...
class IrrecoverableDebtsDisallowableAmountController @Inject() (override val messagesApi: MessagesApi,
  ...
  def onPageLoad(taxYear: TaxYear, businessId: BusinessId, mode: Mode): Action[AnyContent] = (identify andThen getAnswers andThen requireAnswers) {
    implicit request =>
      (for {
        amount <- request.valueOrRedirectDefault(IrrecoverableDebtsAmountPage, businessId)
        form = request.userAnswers
          .get(IrrecoverableDebtsDisallowableAmountPage, businessId.some)
  ...
  def onSubmit(taxYear: TaxYear, businessId: BusinessId, mode: Mode): Action[AnyContent] =
    (identify andThen getAnswers andThen requireAnswers).async { implicit request =>
      ...
      def handleSuccess(answer: BigDecimal): Future[Result] =
        service
          .persistAnswer(businessId, request.userAnswers, answer, IrrecoverableDebtsDisallowableAmountPage)
          .map(answer => Redirect(navigator.nextPage(IrrecoverableDebtsDisallowableAmountPage, mode, answer, taxYear, businessId)))
