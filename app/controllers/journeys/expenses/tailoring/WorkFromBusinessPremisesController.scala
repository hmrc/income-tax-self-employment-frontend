...
class WorkFromBusinessPremisesController @Inject() (override val messagesApi: MessagesApi,
  ...
  def onPageLoad(taxYear: TaxYear, businessId: BusinessId, mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      val preparedForm = request.userAnswers.get(WorkFromBusinessPremisesPage, Some(BusinessId(businessId))) match {
    ...
    implicit request =>
      formProvider(userType(request.user.isAgent))
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(WorkFromBusinessPremisesPage, value, Some(BusinessId(businessId))))
