...
class DisallowableSubcontractorCostsController @Inject() (override val messagesApi: MessagesApi,
  ...
  def onPageLoad(taxYear: TaxYear, businessId: String, mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      val preparedForm = request.userAnswers.get(DisallowableSubcontractorCostsPage, Some(BusinessId(businessId))) match {
    ...
    implicit request =>
      formProvider(userType(request.user.isAgent))
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(DisallowableSubcontractorCostsPage, value, Some(BusinessId(businessId))))
              _              <- sessionRepository.set(updatedAnswers)
            } yield Redirect(navigator.nextPage(DisallowableSubcontractorCostsPage, mode, updatedAnswers, taxYear, BusinessId(businessId)))
        )
  }
