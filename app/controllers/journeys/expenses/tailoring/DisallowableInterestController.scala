...
import views.html.journeys.expenses.tailoring.DisallowableInterestView
...
class DisallowableInterestController @Inject() (override val messagesApi: MessagesApi,
  ...
  def onPageLoad(taxYear: TaxYear, businessId: String, mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      val preparedForm = request.userAnswers.get(DisallowableInterestPage, Some(BusinessId(businessId))) match {
    ...
    implicit request =>
      formProvider(userType(request.user.isAgent))
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(DisallowableInterestPage, value, Some(BusinessId(businessId))))
              _              <- sessionRepository.set(updatedAnswers)
            } yield Redirect(navigator.nextPage(DisallowableInterestPage, mode, updatedAnswers, taxYear, BusinessId(businessId)))
        )
  }
