...
import viewmodels.checkAnswers.expenses.professionalFees._
...
@Singleton
class ProfessionalFeesCYAController @Inject() (override val messagesApi: MessagesApi,
  ...
  def onPageLoad(taxYear: TaxYear, businessId: BusinessId): Action[AnyContent] =
    (identify andThen getUserAnswers andThen getJourneyAnswers[ProfessionalFeesJourneyAnswers](req =>
      req.mkJourneyNinoContext(taxYear, businessId, ExpensesProfessionalFees)) andThen requireData) { implicit request =>
      val summaryList = SummaryListCYA.summaryListOpt(
        rows = List(
          ProfessionalFeesAmountSummary.row(request.userAnswers, taxYear, businessId, request.userType),
          ProfessionalFeesDisallowableAmountSummary.row(request.userAnswers, taxYear, businessId, request.userType)
        )
      )

      Ok(
        view(
          ProfessionalFeesCYAPage.toString,
          taxYear,
          request.userType,
          summaryList,
          routes.ProfessionalFeesCYAController.onSubmit(taxYear, businessId)))
    }
