...
@Singleton
class StructuresBuildingsPreviousClaimedAmountController @Inject() (override val messagesApi: MessagesApi,
                                                                    navigator: CapitalAllowancesNavigator,
                                                                    identify: IdentifierAction,
                                                                    getData: DataRetrievalAction,
                                                                    requireData: DataRequiredAction,
                                                                    service: SelfEmploymentService,
                                                                    formProvider: StructuresBuildingsPreviousClaimedAmountFormProvider,
                                                                    val controllerComponents: MessagesControllerComponents,
                                                                    view: StructuresBuildingsPreviousClaimedAmountView)(implicit ec: ExecutionContext)
    ...
    implicit request =>
      formProvider(request.userType)
        ...
        .fold(
          ...
          value =>
            service
              .persistAnswer(businessId, request.userAnswers, value, StructuresBuildingsPreviousClaimedAmountPage)
              .map(updatedAnswers =>
                Redirect(navigator.nextPage(StructuresBuildingsPreviousClaimedAmountPage, mode, updatedAnswers, taxYear, businessId)))
        )
  }
