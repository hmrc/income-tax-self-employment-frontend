...
import pages.capitalallowances.structuresBuildingsAllowance.StructuresBuildingsQualifyingExpenditurePage
...
class StructuresBuildingQualifyingExpenditureController @Inject() (override val messagesApi: MessagesApi,
  ...
  private val page = StructuresBuildingsQualifyingExpenditurePage
        ...
        .fold(
          ...
          answer =>
            service.updateStructureAnswerWithIndex(request.userAnswers, answer, businessId, index, page).map { _ =>
              page.nextPageWithIndex(request.userAnswers, businessId, taxYear, index)
            }
        )
