...
import views.html.journeys.capitalallowances.writingDownAllowance.WdaMainRateView

import javax.inject.{Inject, Singleton}

@Singleton
class WdaMainRateController @Inject() (override val messagesApi: MessagesApi,
                                       val controllerComponents: MessagesControllerComponents,
                                       identify: IdentifierAction,
                                       getData: DataRetrievalAction,
                                       requireData: DataRequiredAction,
                                       view: WdaMainRateView)
  ...
  def onSubmit(taxYear: TaxYear, businessId: BusinessId, mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      WdaMainRatePage.redirectNext(mode, request.userAnswers, businessId, taxYear)
