...
@Singleton
class AmountSpentOnEvcpController @Inject() (override val messagesApi: MessagesApi,
                                             navigator: CapitalAllowancesNavigator,
                                             identify: IdentifierAction,
                                             getData: DataRetrievalAction,
                                             requireData: DataRequiredAction,
                                             service: SelfEmploymentService,
                                             formProvider: AmountSpentOnEvcpFormProvider,
                                             val controllerComponents: MessagesControllerComponents,
                                             view: AmountSpentOnEvcpView)(implicit ec: ExecutionContext)
  ...
  def onPageLoad(taxYear: TaxYear, businessId: BusinessId, mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      val form = request.userAnswers
        .get(AmountSpentOnEvcpPage, businessId.some)
    ...
    implicit request =>
      formProvider(request.userType)
        ...
        .fold(
          ...
          value =>
            service
              .persistAnswer(businessId, request.userAnswers, value, AmountSpentOnEvcpPage)
              .map(updatedAnswers => Redirect(navigator.nextPage(AmountSpentOnEvcpPage, mode, updatedAnswers, taxYear, businessId)))
        )
  }
